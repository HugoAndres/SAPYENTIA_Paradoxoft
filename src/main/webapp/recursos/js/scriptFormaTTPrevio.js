let numAutores, numCodirectores, numPalabrasClave, numSinodales;
numAutores = numCodirectores = numPalabrasClave = numSinodales = 1;
...
window.onload = function(){
    ...
    document.getElementById("botonMasSinodales").addEventListener("click", function(){agregarSinodal();});
};
...
function agregarSinodal(){
    numSinodales++;
    
    let nuevoCampo = document.createElement("input");
    nuevoCampo.type = "text";
    nuevoCampo.name = "sinodal" + numSinodales;
    nuevoCampo.required = "required";
    ...
    let textoEquis = document.createTextNode("✕");
    nuevoBoton.appendChild(textoEquis);
    nuevoBoton.addEventListener("click", function(){quitarSinodal(event);});
    ...
    let camposSinodales = document.getElementById("camposSinodales");
    camposSinodales.appendChild(nuevoCampoConBoton);
}

function quitarSinodal(event){
    numSinodales--;
    let botonClicado = event.target;
    let campoConBotonRelacionado = botonClicado.parentElement;
    campoConBotonRelacionado.remove();
    ...
    if(numSinodales !== 1){
        let camposSinodales = document.getElementById("camposSinodales");
        let camposConBoton = camposSinodales.children;
        let numCamposConBoton = camposConBoton.length;
        for (let i = 1; i < numCamposConBoton; i++)
            camposConBoton[i].querySelector("input").name = "sinodal" + (i+1);
    }
}